...
| v1.18.0 | ヘッダーアイコン群を左サイド縦並びに移行 (planned): PNG モック準拠で旧の右上横並びアイコン (言語 / テーマ / 設定 / 閉じる) を **左サイド縦並び** に移行 (Q7-a 推奨)。 デスクトップ・オフラインブラウザでこの種のツールを初めて触るユーザにも「アイコン = 機能」 の対応が分かりやすい配置を採用。 モバイルレイアウト時は下部ナビに移行する想定 (PNG モック明示)。 |
| v1.19.0 | 旧 Export* メッセージ系統の整理 (planned): v1.16.0 で `convert_in_memory_task` 経路が主導線になったが、 旧 `ExportRequested` / `ExportDirPicked` / `ExportCompleted` は phase A 互換のため残置中。 Result 画面に到達する経路を `ConvertCompleted` のみに収束させ、 旧パスを削除する。 `exporter::run_in_memory` 直接 API もこのタイミングで導入を検討。 |
