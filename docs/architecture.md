...
| v1.14.0 | Dark-mode color integrity + visual hierarchy (planned): currently many UI colors are defined as `Color::from_rgb(...)` constants (APP_NAME_COLOR, TAGLINE_COLOR, MUTED_TEXT, HEADING_COLOR, etc.). These were tuned for the light theme; on a dark background they end up too close to the surface to be legible. v1.14 routes most of these through `theme.extended_palette()` so they invert with the theme, while keeping the few colors that should *not* invert (the transparency-checker grey/white squares — they ARE the indicator) explicitly hardcoded with a comment marker. Also tightens the cross-screen visual hierarchy: title sizes, accent vs auxiliary button styling, and helper text contrast across the empty / preview / export-ready screens. Aim is that switching `Theme::System / Light / Dark` from the header icon produces a coherent rendering at every screen, including transparency reports and warning toasts. |
| v1.15.0 | Advanced drawer scroll + sticky footer (planned): when many sections are expanded the drawer's Reset/Close footer can fall below the bottom of the sheet and become unreachable. v1.13.0's snora upgrade made the `Sheet` a pure content carrier, so the fix is now self-contained in logolig: wrap the drawer body in `iced::widget::scrollable`, and place the Reset/Close row outside that scroll area so it stays sticky to the sheet bottom. Reset will also be repositioned to the left (auxiliary, controlled muted) with Close on the right (primary). |
| v1.16.0 | Startup screen redesign (planned): the empty/drop-zone screen still shows the regular header (app name + tagline + four icon buttons) at full strength. With v1.12.0 making the edit screen subject-led ("the file is the protagonist"), the startup screen feels relatively over-loaded. v1.16 will quiet the startup header (smaller / fainter, or fold the icons into a corner overflow) and grow the drop card's visual weight so the "drop a file here" path is the only thing on the screen demanding attention. Drag-over feedback (background tint when a file is hovering above the window) will be added if the iced 0.14 drag-and-drop surface API supports a hover state. |
