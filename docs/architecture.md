...
| v1.11.0 | Advanced settings information design (planned): the current advanced drawer lists six sections in implementation order rather than user-need order, and within each section labels and controls are inconsistently spaced. Need to (1) classify settings into output-shape concerns (which artifacts, which sizes), processing concerns (resize algorithm, vtracer preset), and meta concerns (language) and (2) order them along the user's expected flow — what gets written first, then how it gets rendered, then app-wide preferences last. Likely outcomes: re-grouped sections with clearer headings, possibly a two-column layout for the chip-style size editors, and a quiet visual hierarchy that makes "you usually don't need to touch this" obvious without being grey-on-grey illegible. The change is presentation-only; `ExportPlan` and `MessageKey` stay stable. |
