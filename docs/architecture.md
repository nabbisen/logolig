...
| **v1.17.0** | **設定ドロワー Right Sheet 化 + flat 再編**: 旧 Bottom Sheet を Right Sheet (`SheetEdge::End`) に移行、 中身を PNG モック準拠の flat なセクション構成に作り直した。 PNG モックの「設定」 ドロワーには 4 セクション (出力サイズ (PNG) / ICO 生成 / SVG 変換方式 / その他) のみ含まれていたが、 旧 logolig が持つ追加機能 (Apple touch icon / HTML snippet / Web manifest / Monochrome / リサイズアルゴリズム / vectorize_on_raster) は **「上級設定」 折りたたみ** (chevron `▶` / `▼`) に集約して残した (= モックに無い設定の救済策、 通常ユーザにはモック準拠の見た目のまま、 上級ユーザにはアクセス経路を確保)。 Q4 で確定した方針により **ICO 生成セクションは完全撤去**、 favicon.ico は内部デフォルト固定 (16/24/32/48 マルチサイズ) で常時 ON、 ユーザは触らない (`.ico` の現代的価値はほぼ「IE / 古いブラウザ互換」 のための慣例で、 サイズ構成にユーザが介入する必要はない)。 Q3 で確定した方針により出力サイズは「PNG モック準拠の固定 6 サイズ (16/32/48/96/192/512) チェックボックス + 『+ カスタムサイズ追加』 入力欄」 のハイブリッド (固定リストの整然さ + 既存の柔軟性を両立)。 SVG 変換方式は「離散スライダー 3 段 (シンプル ↔ 詳細)」 で旧 vtracer プリセット (Sharp / Default / PhotoRich) と直接対応 — `iced::widget::slider(0..=2, ...).step(1i32)` で離散値、 ABDD §12 (色覚に依存しない) を維持しつつ「精度を 1 軸の連続感」 で見せる。 Right Sheet の幅は **`window_width / 3.0` を `[280px, 480px]` で clamp** した動的計算 (`shell::drawer_pixel_width`) で、 小さい画面ではラベルが読める下限を確保し、 大きい画面では中央コンテンツを圧迫しない上限を設ける (Q5 で確定 — snora 0.8 の `SheetSize` API には min/max を組合せる仕組みが無いため、 アプリ側で算出して `Pixels(...)` を渡す)。 ウィンドウサイズ取得のため `iced::window::events()` の `Resized` / `Opened` を subscribe し、 `Message::WindowResized(iced::Size<f32>)` 経由で `AppState::window_size` を更新。 フッタは「↻ Reset」 のみ左寄せ、 Close は右上の ✕ に統合 (Q6-b、 PNG モック準拠)。 旧 `accordion_group` / `size_subsection` / `size_set_editor` / `size_chip` ヘルパ、 旧 `AdvancedGroup` / `AdvancedGroupExpansion` / `AdvancedGroupToggled` / `advanced_groups` フィールド、 旧 `colors::badge_muted_bg`、 旧 `close_button_style` を全削除し、 v1.17.0 用の新スタイル (`title_close_button_style`, `extras_header_style`, `secondary_drawer_button_style`) を追加。 9 個の MessageKey 追加 + en/ja 翻訳。 透過維持トグルは v1.17.0 では UI のみ用意 (常時 true、 placeholder)、 実装は v1.17.x で `ExportPlan::keep_transparency` を追加して対応予定。 |
| **v1.18.0** | **ヘッダーアイコン群を左サイド縦並びに移行**: PNG モック (新外部設計) 準拠で旧の右上横並びアイコン (言語 / テーマ / 設定 / 閉じる) を **左サイド縦並び** に移行 (Q7-a)。 ヘッダー上部はロゴ + アプリ名 (Empty/Converting) または ファイル名 (Result) のみに簡素化。 「閉じる」 ボタンは完全撤去 (Q2-a) — OS のネイティブウィンドウチャートに任せる方針 (ブラウザ移行を視野に入れた決定、 アプリ内 ✕ は冗長なため)。 アイコンは **snora 0.8 の `widgets` + `lucide-icons` features 経由で lucide-icons** を採用 (Q3): `lucide::Settings` (歯車) / `lucide::Languages` (言語切替) / `lucide::Moon` (テーマ)。 旧 `language_icon_glyph()` / `theme_icon_glyph()` の cycle UI 自前実装は廃止。 言語/テーマピッカーは PNG モックの「言語メニュー」 / 「テーマメニュー」 オーバーレイに対応するため、 **snora の `context_menu` slot** をクリック型ポップアップとして流用 (`AppLayout::context_menu(...)` + `on_close_menus(SidebarPickerClosed)`、 外側クリックで閉じる)。 1 度に 1 種類しか開けない (snora の `context_menu` は単数 slot)。 cycle 廃止により `LocalePicked(Option<Locale>)` / `ThemePicked(ThemeMode)` Message を新設 — 直接選択 (✓ プレフィックスで現在値を視覚化)。 サイドバーは PNG モック準拠の **90px 幅、 アイコン (22px) + 下にラベル文字 (11px)** の縦 2 段構成 (snora 標準の `app_side_bar` widget は 64px + tooltip のみで PNG モックの「アイコン下ラベル」 と一致しないため、 `Icon` データ型 + `icon_element_sized` 関数のみ流用しつつレール本体は logolig 側で組み立て)。 active state (= 関連ピッカー or ドロワー が開いている) は `palette.background.strong.color` を alpha 0.35 で薄塗り + ラベル色を `page_title` に切替で視覚化 (ABDD §12 「色覚に依存しない」 を維持)。 `AppLayout::side_bar(...)` slot 採用により snora のスケルトン (header / side_bar | body / footer) に統合され、 LTR/RTL の自動配置にも対応 (将来 RTL 言語追加時に start edge が自動的に右に切替)。 9 個の MessageKey 追加 (SidebarLabelSettings / SidebarLabelLocale / SidebarLabelTheme / LocaleNameJa / LocaleNameEn / LocaleSystem / ThemeNameLight / ThemeNameDark / ThemeSystem) + en/ja 翻訳。 旧 `Message::AppCloseRequested` ハンドラは v1.18 で参照しなくなったため将来 v1.19 で削除候補に。 中国語 (简体中文) のロケール追加は別フェーズに繰下げ (Q4-c)。 |
| **v1.19.0** | **旧 Message 系統の整理 + `exporter::run_in_memory` 直接 API**: v1.16.0 で `convert_in_memory_task` 経路が主導線になって以降、 旧 Export* / Cycle 系 Message が dead code として残置されていたのを **完全削除**。 削除対象 6 種: `ExportRequested` / `ExportDirPicked(Option<PathBuf>)` / `ExportCompleted(Result<ExportReport, AppError>)` (旧「Export ボタン → ディレクトリ選択 → 一括書出」 動線、 v1.16.0 で個別 DL / ZIP DL に置換)、 `LocaleCycled` / `ThemeToggled` (旧ヘッダ右上アイコンの cycle UI、 v1.18.0 で left sidebar + ピッカーポップアップに置換され `LocalePicked` / `ThemePicked` が後継)、 `AppCloseRequested` (旧 ✕ ボタン、 v1.18.0 で完全撤去 = OS ネイティブに任せる方針)。 関連ハンドラ全削除、 `task_queue::pick_export_dir_task` / `task_queue::export_task` 削除、 `ui::preview_panel::view` の旧 action_row (戻る / 再選択 / Export ボタン) と `secondary_button_style` も orphan として削除。 `logolig-core` 側の主要変更: **`exporter::run_in_memory(asset, plan) -> Result<Vec<InMemoryArtifact>, AppError>` 直接 API を新設**。 `pub struct InMemoryArtifact { pub relative_path: PathBuf, pub bytes: Vec<u8> }` の Vec を返す pure in-memory 実装で、 ディスク I/O 一切なし。 favicon 一式の合計バイト数は通常 1 MB 未満なのでメモリ保持コストは実質ゼロ。 副次効果として **ブラウザ移行 (file system API なしで動く)** が視野に入り、 一時ディレクトリ後始末漏れ (途中 panic 等) リスクもゼロ化。 旧 `exporter::run(asset, plan, output_dir)` (ディスク版) は signature 不変のまま **`run_in_memory` の薄いラッパに整理** — 内部で `run_in_memory` を呼び、 戻った Vec を staging dir 経由で atomic に書き出す。 既存 12 テスト (`tests/exporter.rs` の disk 版) は無修正で全て通る。 `logolig-app` の `task_queue` 側: 旧 `convert_in_memory_task` (v1.16 で OS 一時ディレクトリ経由で実装していたもの) を **`convert_task` に rename** し、 内部実装を `exporter::run_in_memory` 直接呼び出しに簡素化 — 一時ディレクトリ作成 / 書出 / 読み戻し / 後始末の 4 ステップが消滅。 `ResultAsset` への変換 (種別判定 / 寸法解析 / サムネ生成) は logolig-app 側に留まる (関心の分離を維持)。 `lib.rs` で `InMemoryArtifact` を `ExportReport` と並列に re-export (`pub use services::exporter::{ExportReport, InMemoryArtifact};`)。 新規ユニットテスト 5 個追加 (`tests/exporter.rs` 末尾): `run_in_memory_returns_default_artifact_set_from_png_source` / `run_in_memory_bytes_match_disk_run_byte_for_byte` / `run_in_memory_encodes_mono_subdirectory_in_relative_path` / `run_in_memory_returns_err_on_empty_ico_sizes` / `run_in_memory_skips_optional_artifacts_when_disabled`、 これらで in-memory API の戻り値の本数 / 順序 / 相対パス / バイト一致 / mono サブディレクトリ表現 / エラーケース / オプション設定の連動 を網羅。 |
...
| v1.21.0+ (option) | モバイル UX のさらなる改善 (planned): (a) ピッカーポップアップ幅のモバイル拡張 — 現状 220px 固定だが、 v1.20.0 の Q6 で `window_width - 32px` への動的化を将来候補として保留した。 サイドバーアイコン隣への正確な配置 (overlay positioning) も含めて検討。 (b) `Bottom Sheet` 風ピッカー — snora の `context_menu` slot は単数のため設定 Right Sheet との同居不可だが、 picker_overlay の表示形式を「画面下から滑り上がる」 形に変える代替案として検討余地あり。 (c) ハンバーガーメニュー (PNG モック明示) — 下部ナビで多くのケースは賄えるため必須性は低いが、 将来の機能追加 (4 つ目以降のアイコン) で必要になれば追加。 |
| v1.22.0+ (option) | BMP 入力対応 / 中国語ロケール追加 / 透過維持本実装 (planned): BMP は `image` crate の `bmp` feature を有効化するだけで実装は軽い。 中国語 (簡体中文) ロケール追加は v1.6.0 と同じ規模 (~100 翻訳キー) の作業。 透過維持トグルの本実装 (`ExportPlan::keep_transparency` 追加 + デコード時の挙動切替) は v1.17.0 で UI placeholder のみ用意済。 |
