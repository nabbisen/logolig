...
└── crates/
    ...
    └── logolig-app/                 # iced + snora GUI binary
        ...
        │       ├── sidebar.rs          # Left sidebar (since v1.18)
...
| v1.19.0 | 旧 Export* メッセージ系統の整理 (planned): v1.16.0 で `convert_in_memory_task` 経路が主導線になったが、 旧 `ExportRequested` / `ExportDirPicked` / `ExportCompleted` は phase A 互換のため残置中。 v1.18 で参照されなくなった `Message::AppCloseRequested` / `Message::LocaleCycled` / `Message::ThemeToggled` も整理候補。 Result 画面に到達する経路を `ConvertCompleted` のみに収束させ、 旧パスを削除する。 `exporter::run_in_memory` 直接 API もこのタイミングで導入を検討。 |
| v1.20.0+ (option) | モバイルレイアウト対応 / BMP 入力 (planned): 横幅で flex する振る舞い、 下部ナビゲーション、 ハンバーガー メニューなど (PNG モックのモバイル表示準拠)。 BMP は `image` crate の `bmp` feature を有効化するだけで実装は軽い。 |
