...
| **v1.25.0** | **snora 0.25 upgrade (Snora Design System available, not yet adopted)**: snora dependency bumped from 0.18 → 0.25. The 0.18→0.25 range is additive for logolig; the only breaking change (0.24 made `Palette::roles()` test-only) does not affect logolig, which uses none of the design system. Every snora API logolig depends on (`AppLayout` builder, `Icon`, `lucide`, `Toast`/`ToastIntent`/`ToastLifetime`, `LayoutDirection`, `render`, `toast::subscription`/`sweep_expired`, `widget::icon_element_sized`) is unchanged. Notable: 0.18.1 fixed the `Icon::Lucide` rendering compile path that logolig's lucide icons rely on. The new opt-in `design` feature (iced-free token crate + button/card helpers + notice/chip/progress primitives with WCAG-AA-verified contrast and four presets including high-contrast) is **not enabled**: it is opt-in by snora's own policy (default-on pending a size/build-cost review), and migrating logolig's hand-written `button::Style` closures to the token system is optional UI/UX work, not a requirement of the version bump. No code changes; version-string bump only. |
| **v1.26.1** | **Drop-zone drag/drop repair**: switched runtime event handling to `iced::event::listen_with`, added native file-hover feedback, and made the whole Empty-screen drop-zone card a file-picker button so the choose-file page remains usable when native OS file drop is unavailable. |
| **v1.26.2** | **Stability patch**: release-gate and docs stabilization after v1.26.1. The app registers the lucide icon font explicitly so snora lucide buttons render real icons, ZIP bundle entries now carry local wall-clock modified timestamps, and Wayland drag-hover visual feedback is tracked as non-blocking future work in RFC 0007. |
| **v1.26.0** | **Microsoft app logos**: added an advanced, opt-in `ExportPlan::include_microsoft_app_logos` flag that generates `StoreLogo.png` (50×50), `Square44x44Logo.png` (44×44), `Square150x150Logo.png` (150×150), and `Wide310x150Logo.png` (310×150). The implementation keeps favicon defaults unchanged, stores the setting through the existing persisted `ExportPlan`, adds en/ja i18n strings, and uses a new contain-fit canvas helper for non-square outputs so the wide logo is centred without cropping or stretching. |
| v1.26.0+ (option) | Optional features (planned): (a) **Adopt the Snora Design System** — enable the `design` feature and migrate the hand-written `button::Style` closures across the UI to `snora::design` token-based helpers (`button::primary/secondary/ghost/danger`), gaining the four built-in presets (incl. high-contrast light/dark) and WCAG-AA contrast for free; consider the `notice` primitive for the JPEG transparency advisory and `progress` for the Converting screen. Deliberate, reviewable step per snora policy. (b) **Mobile UX refinements** — dynamic picker popup width (`window_width - 32 px`), accurate icon-adjacent positioning, bottom-sheet-style picker, hamburger menu (when a 4th sidebar icon is added). (c) **User-specified flatten colour** — v1.21.0 is white-only; black or arbitrary colour (`flatten_color: [u8; 3]`) is a straightforward extension. (d) **BMP input** — enable the `image` crate `bmp` feature; low implementation cost. (e) **Simplified Chinese locale** — same scope as the v1.6.0 Japanese addition (~100 translation keys). | — dynamic picker popup width (`window_width - 32 px`), accurate icon-adjacent positioning, bottom-sheet-style picker, hamburger menu (when a 4th sidebar icon is added). (b) **User-specified flatten colour** — v1.21.0 is white-only; black or arbitrary colour (`flatten_color: [u8; 3]`) is a straightforward extension. (c) **BMP input** — enable the `image` crate `bmp` feature; low implementation cost. (d) **Simplified Chinese locale** — same scope as the v1.6.0 Japanese addition (~100 translation keys). |

### Implementation handoff: `rfcs/`

Detailed implementation specs for the `v1.26.0+ (option)` themes above
